...
│       ↓                                                         │
│  Step 1.5: 符号对账                                              │
│          （binary nm/objdump ↔ 头文件声明逐行 diff，差异清零）    │
│       ↓                                      ← ⚠ 确认点 1.5【阻断门】│
...
│                                                                 │
│  Step 6.5: 接口重设计工作坊                                       │
│          （层 1：数据类型层 → 层 2：实现依赖层 → 层 3：顶层接口层） │
│       ↓                                      ← ⚠ 确认点 5【阻断门】│
│  Step 7: 提出变更提案（每项 CHG-xxx）         ← ⚠ 确认点 6【阻断门】│
│       ↓                                                         │
...
### Step 1.5：符号对账 → ⚠ 人工确认点 1.5【阻断门】

**目标**：将 `scan_headers.py` 的头文件声明结果与从已构建二进制实测的 `nm`/`objdump` 导出符号进行逐行 diff，消除所有差异歧义，最终锁定北向接口的精确集合。

> **为什么需要这一步？**
> `scan_headers.py` 只能读取头文件声明，无法感知条件编译、链接脚本、宏生成等导致的
> 实际导出差异。跳过此步骤会导致 Rust 侧实现的接口数量与实际二进制导出不匹配。

1. 运行 `nm`/`objdump` 提取已构建二进制的导出符号（与 Step 0 第 4 节一致）。
2. 打开 `templates/abi-freeze-checklist.md` 第 2 节，填写符号对账差分表（第 2.2 节）：
   - **匹配**行：正常纳入北向接口。
   - **仅头文件有**行：逐一解释原因（内部函数 / 条件编译排除 / 尚未构建）。
   - **仅二进制有**行：逐一解释原因（隐式导出 / 宏生成 / 已纳入 Rust 侧导出计划）。
3. 填写差分表统计，确认「最终确认的北向接口数量」，该数字将作为后续步骤（Step 3 ABI 冻结、Step 5 Rust 实现）的基准。

**确认要求（必须全部完成，不得绕过）**：
- [ ] `nm`/`objdump` 原始输出已粘贴至 `abi-freeze-checklist.md` 第 2.1 节
- [ ] 第 2.2 差分表中每一行的「处置策略」均已填写（无 TODO 行）
- [ ] 第 2.2 差分表中每一行的「已对账」均已勾选
- [ ] 差分统计表中「最终确认的北向接口数量」已填写
- [ ] 负责人已签字

**输出物**：`abi-freeze-checklist.md` 第 2 节（符号对账部分已完成并签字）

> ⚠ **阻断门**：符号对账未完成签字之前，**不得推进至 Step 2**。
...
### Step 6.5：接口重设计工作坊 → ⚠ 人工确认点 5【阻断门】

**目标**：在开始填写 CHG-xxx 提案之前，从底层到顶层整体审视现有设计，确保 Phase 2 的变更具有内聚性，而非各接口孤立改造。

> **为什么需要这一步？**
> 按接口逐个击破容易出现"底层类型改了但顶层形状没变"或"顶层改了但底层结构不支持"的问题。
> 三层整体分析可以在动手之前识别系统性设计机会，并作为后续 CHG-xxx 的蓝图依据。

打开 `templates/phase2-change-proposal.md` 的「接口重设计工作坊」节，完成三层分析：

1. **层 1：数据类型层重审** — 逐一审查跨 FFI 边界的数据类型，评估从 C 风格类型迁移到 Rust 惯用类型的机会（如 `int` 错误码 → `Result`、`void*` → RAII 句柄、`char*` → `&str`/`String`）。
2. **层 2：实现依赖层重审** — 逐函数审查内部实现依赖，识别 C 特有的隐含依赖（全局状态、errno、手动生命周期），评估在 Rust 侧如何更自然地表达（`OnceLock`、`Drop`、`Mutex` 等）。
3. **层 3：顶层接口架构重审** — 基于前两层结论，整体规划模块边界、API 粒度、trait 设计和命名空间，产出「Phase 2 公开接口蓝图」。

**确认要求（必须全部完成，不得绕过）**：
- [ ] 数据类型层：所有跨 FFI 数据类型已逐行审查并有明确结论
- [ ] 实现依赖层：所有北向函数的 C 特有依赖已识别，Rust 替代方案已确认
- [ ] 顶层接口层：Phase 2 公开接口蓝图已填写
- [ ] 工作坊节中无空白字段（无 TODO 行）
- [ ] 主导设计人和审查人已签字

> ⚠ **阻断门**：工作坊签字完成之前，**不得开始填写 CHG-xxx 提案**。

---

### Step 7：Phase 2 变更提案 → ⚠ 人工确认点 6【阻断门】

参照 `templates/phase2-change-proposal.md`，为每个计划变更创建提案（CHG-xxx）：

- 每项提案包含：变更动机、旧行为、新行为、破坏等级、迁移方式、验收测试。
- **所有 CHG-xxx 必须与 Step 6.5 工作坊产出的「Phase 2 公开接口蓝图」一致**；蓝图外的变更不得引入。
- 所有提案汇总后，由用户/产品负责人逐项审查并决策（接受 / 拒绝 / 延后）。
- **确认要求（必须全部完成，不得绕过）**：
  - [ ] 每个 CHG-xxx 提案均有完整填写（无空白字段）
  - [ ] 每个 CHG-xxx 已有明确的接受/拒绝/延后决策
  - [ ] 被接受的 CHG-xxx 均有对应的验收测试描述
  - [ ] 所有 CHG-xxx 均可追溯到 Step 6.5 工作坊蓝图中的具体条目
  - [ ] 产品/用户负责人已逐项签字确认
...
| ⚠ 1.5 | Step 1 完成后 | 符号对账：binary nm/objdump ↔ 头文件声明逐行 diff，差异全部清零 | `abi-freeze-checklist.md` 第 2 节 | **是** |
| ⚠ 1 | Step 2 完成后 | 风险信号处置策略（每项必填决策） | `generate_report.py` 输出 | **是** |
| ⚠ 2 | Step 3 完成后 | ABI / FFI 冻结（符号清单须来自实测，与对账结果一致） | `abi-freeze-checklist.md` | **是** |
...
| ⚠ 5 | Step 6.5 完成后 | 接口重设计工作坊三层分析（数据类型层→实现依赖层→顶层接口架构） | `phase2-change-proposal.md` 工作坊节 | **是** |
| ⚠ 6 | Step 7 完成后 | Phase 2 变更逐项确认（须与工作坊蓝图一致） | `phase2-change-proposal.md` | **是** |
