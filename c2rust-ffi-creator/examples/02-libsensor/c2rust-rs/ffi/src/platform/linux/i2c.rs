...
extern "C" {
    fn i2c_open(bus_path: *const c_char) -> c_int;
    fn i2c_close(fd: c_int) -> ();
    fn i2c_write(fd: c_int, addr: u8, data: *const u8, len: usize) -> c_int;
    fn i2c_read(fd: c_int, addr: u8, buf: *mut u8, len: usize) -> c_int;
}
