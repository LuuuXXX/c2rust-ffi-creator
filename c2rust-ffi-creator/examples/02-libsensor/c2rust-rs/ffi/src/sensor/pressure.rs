...
mod sys {
    ...
    use super::*;
    extern "C" {
        pub(super) fn pressure_sensor_init(addr: u8) -> c_int;
        pub(super) fn pressure_sensor_read(addr: u8, out: *mut PressureReadingT) -> c_int;
        pub(super) fn pressure_sensor_shutdown(addr: u8);
    }
}
...
pub unsafe extern "C" fn pressure_sensor_read(addr: u8, out: *mut PressureReadingT) -> c_int {
    sys::pressure_sensor_read(addr, out)
}
...
pub unsafe extern "C" fn pressure_sensor_shutdown(addr: u8) {
    sys::pressure_sensor_shutdown(addr)
}
